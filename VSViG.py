...
class InterPartMR(nn.Module):
    ...
    def forward(self,x):
        ...
        # Original: tmp_x = x
        tmp_x = x.clone() # [Fix 1] Clone to avoid side effects
        ...
        for part in range(5):
            # [Fix 2] CRITICAL BUG FIX
            # Original: tmp_relative = relative (Reference only!)
            # Fixed: .clone() ensures we don't overwrite 'relative' for the next loop
            tmp_relative = relative.clone() 
            
            # Mask out the CURRENT partition (Self) so we look at INTER (Others)
            # This implements E_inter from Eq 2
            start_idx = part * 3
            end_idx = (part + 1) * 3
            tmp_relative[:,:,:,start_idx:end_idx] = relative[:,:,:,start_idx:end_idx] - 1e4
            
            # Find max relative feature
            tmp_x_j, _ = torch.max(tmp_relative, -1, keepdim=True)
            tmp_x[:,:,start_idx:end_idx,:] = tmp_x_j[:,:,start_idx:end_idx,:]
...
class IntraPartMR(nn.Module):
    ...
    def forward(self,x):
        B, C, P, _ = x.shape # BxT, C, P, 1
        #tmp_x = x
        tmp_x = x.clone() # Good Practice to avoid side effects
        ...
        # Implements E_intra: Only look at neighbors within same partition
        part = 1
        for point in range(P):
            # Logic assumes 3 points per partition (15 total)
...
class Stem_pe(nn.Module):
    def __init__(self, input_dim=3, output_dim=None, patch_size=32): # 32
        super().__init__()
        # Implements "Stem" Positional Embedding
        self.stem = nn.Sequential(
...
class Part_3DCNN(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, dynamic=False, dynamic_point_order=None, SEED=None, expansion=4):
        ...
        # Temporal Modeling with 3D CNN
        self.conv1 = nn.Sequential(
            nn.Conv3d(in_channels, out_channels, kernel_size=1),
            nn.BatchNorm3d(out_channels),
            nn.ReLU()
        )
        # Kernel (3,3,1) -> (Time=3, Space=3, 1) matches Ks=3, Kt=3
        self.conv2 = nn.Sequential(
            nn.Conv3d(out_channels, out_channels, kernel_size=(3,3,1), stride=stride, padding=(1,1,0), padding_mode='replicate'),
            nn.BatchNorm3d(out_channels),
            nn.ReLU()
        )
        # Original: padding = 1, thus the change of x = x[:,:,:,:,1].unsqueeze(-1)
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)
        x = self.conv2(x)
        # Original: x = x[:,:,:,:,1].unsqueeze(-1)
        x = self.conv3(x)
        
        # Handle residual path stride
        if residual.shape != x.shape:
             residual = self.downsample(residual)
        # Original: residual = self.downsample(residual) 
        x = residual + x
        x = self.act(x)
...
class STViG(nn.Module):
    def __init__(self, opt):
        super().__init__()
        #height = 1080, width = 1920, T = 30, points = 15, scale = 1/4
        dynamic = opt.dynamic
        ...
        self.backbone = nn.ModuleList() # Use ModuleList for proper registration

        # Build 4 Stages
        for stage in range(len(num_layer)):
            # Downsampling transition (except first stage)
            if stage > 0:
                self.backbone.append(Grapher(in_channels=self.in_channels, out_channels=output_channels[stage]))
                self.backbone.append(Part_3DCNN(stride=(2,1,1), # Temporal downsample
        ...
        self.backbone_seq = nn.Sequential(*self.backbone)
        
        # Classifier Head
        self.fc = nn.Sequential(nn.Conv2d(output_channels[-1] * expansion, 256, 1),
                                nn.BatchNorm2d(256),
                                nn.ReLU(),
                                nn.Conv2d(256, 1, 1) # 2 classes Output: 1 scalar (Regression/Prob)
                                ) 
    ...
    def model_init(self):
        for m in self.modules():
            if isinstance(m, (nn.Conv3d, nn.Conv2d)):
                ...
                nn.init.constant_(m.bias, 0.0) # Set bias (beta) to 0
                nn.init.constant_(m.weight, 1.0) # Set weight (gamma) to 1
    
    def pe(self, flag, x, kpts=None):
        B, T, P, C = x.shape
        if flag == 'learn':
            # [Fix 3] Device compatibility: .to(x.device)
            #Original: x = x + nn.Parameter(torch.zeros(1, T, P, C)).cuda()
            x = x + nn.Parameter(torch.zeros_like(x)).to(x.device)
        elif flag == 'add':
...
def VSViG_base(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, **kwargs):
            self.dynamic = 1
            self.num_layer = [2,2,6,2] # Matches Table 5
